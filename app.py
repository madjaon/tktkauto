...
# ==========================================================
# (Giữ nguyên các hàm: get_random_background_image, get_random_pexels_image, get_random_unsplash_image,
# text_wrap, create_image_with_text, upload_to_drive, send_telegram_notification, create_drive_folder)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(FONT_PATH):
        print(f"⚠️ LỖI: Không tìm thấy file font '{FONT_PATH}'. Vui lòng tải một file font (.ttf) và đặt tên file là 'font.ttf'.")
        exit()
    if not GEMINI_API_KEY or GEMINI_API_KEY == "YOUR_GEMINI_API_KEY":
        print("⚠️ LỖI: GEMINI_API_KEY chưa được thiết lập.")
        exit()
    if not os.path.exists(CLIENT_SECRETS_FILE):
        print(f"⚠️ LỖI: Không tìm thấy file cấu hình ứng dụng: {CLIENT_SECRETS_FILE}")
        exit()
    ...
    try:
        ...
        print(f"⚠️ LỖI TOKEN GẶP PHẢI: {e}. Bắt buộc phải xác thực lại.")
        ...
        try:
            ...
            if gauth.credentials:
                ...
                print("❌ Xác thực Drive thất bại.")
                exit()
        except Exception as e:
            print(f"❌ Lỗi nghiêm trọng khi xác thực Drive: {e}")
            exit()
...
while True:
    ...
    if app_domains:
        ...
        print(f"✅ Đã chọn Lĩnh vực/Chủ đề: **{chosen_domain}**")
        ...
        try:
            ...
            print(f"❌ Lỗi nghiêm trọng trong quá trình chạy ứng dụng {app_name}: {e}")

        # D. Tùy chọn tiếp tục hoặc dừng hẳn (CHỈ HỎI KHI CHẠY THÀNH CÔNG/GẶP LỖI SAU KHI CHỌN DOMAIN)
        while True:
            choice = input("Bạn có muốn tiếp tục chạy một vòng lặp ngẫu nhiên nữa không? (y/n): ").strip().lower()
            if choice == 'n':
        ...
        print(f"❌ Lỗi: Ứng dụng '{app_name}' không có danh sách lĩnh vực/chủ đề được định nghĩa. Tự động chuyển sang vòng lặp mới sau 2 giây.")
        time.sleep(2)
